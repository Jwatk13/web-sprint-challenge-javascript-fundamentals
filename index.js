...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooArray){
    const displayNames = [];
   zooArray.forEach((item) => {
   const newItem = `name: ${item.animal_name}, scientific: ${item.scientific_name}`
   displayNames.push(newItem);
   });
   return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(zooArray){
    const newName = zooArray.map((itemName) => {
     return itemName.animal_name.toLowerCase();
    });
    return newName;
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooArray){
   const popConcern = zooArray.filter((itemPop) => {
      return itemPop.population < 5;
    });
    return popConcern;
  }
  ...
  */
  ...
  function USApop(zooArray){
    const populationList = zooArray.map(pop => pop.population);

    const totalPop = populationList.reduce((total, original) => {
  ...
  Use the higher-order function called consume to do the following:
    1. Receive 3 parameters: a, b and cb. The first two parameters (a and b) can take any argument (we can pass any value as an argument) and the 
    last parameter (cb) accepts a callback
...
function greeting(stringA, stringB){
   return `Hello ${stringA} ${stringB}, nice to meet you!`;
  }
...
//🐴🐴🐴 Task: You are to build a cuboid maker that can return values for a cuboid's volume or surface area. Cuboids are similar 
//to cubes but do not have even sides. Follow the steps in order to accomplish this challenge. 🐴🐴🐴
